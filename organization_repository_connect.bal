...
isolated int createOperationNextIdOrg = 50;

public isolated function createOrg(uscore501:USCoreOrganizationProfile organization) returns r4:FHIRError|uscore501:USCoreOrganizationProfile {
    ...
    if claim is error {
        ...
        lock {
            claim.id = (++createOperationNextIdOrg).toBalString();
        }
...
public isolated function getByIdOrg(string id) returns r4:FHIRError|uscore501:USCoreOrganizationProfile {
    lock {
...
public isolated function updateOrg(json payload) returns r4:FHIRError|fhir:FHIRResponse {
    return r4:createFHIRError("Not implemented", r4:ERROR, r4:INVALID, httpStatusCode = http:STATUS_NOT_IMPLEMENTED);

}

public isolated function patchResourceOrg(string 'resource, string id, json payload) returns r4:FHIRError|fhir:FHIRResponse {
    return r4:createFHIRError("Not implemented", r4:ERROR, r4:INVALID, httpStatusCode = http:STATUS_NOT_IMPLEMENTED);
}

public isolated function deleteOrg(string 'resource, string id) returns r4:FHIRError|fhir:FHIRResponse {
    return r4:createFHIRError("Not implemented", r4:ERROR, r4:INVALID, httpStatusCode = http:STATUS_NOT_IMPLEMENTED);

}

public isolated function searchOrg(map<string[]>? searchParameters = ()) returns r4:FHIRError|r4:Bundle {
    r4:Bundle bundle = {
        'type: "collection"
    };
    ...
    if searchParameters is map<string[]> {
        ...
        foreach var 'key in searchParameters.keys() {
            match 'key {
                "_id" => {
                    uscore501:USCoreOrganizationProfile byId = check getByIdOrg(searchParameters.get('key)[0]);
                    bundle.entry = [
                        {
                            'resource: byId
                        }
...
function initOrg() returns error? {
    lock {
